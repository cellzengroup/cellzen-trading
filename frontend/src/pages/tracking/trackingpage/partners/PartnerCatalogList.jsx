...
export default function PartnerCatalogList() {
  ...
  useEffect(() => {
    const loadSharedProducts = async () => {
      try {
        ...
        // Filter products that are actually shared with partners
        const sharedProducts = (result.data || []).filter(p =>
          p.share_to && p.share_to.partners === true
        );
  ...
  return (
    <PartnersPortal activePage="Catalogs">
      <div className="flex h-full flex-col overflow-hidden rounded-[2rem] bg-white px-5 py-5 text-[#2D2D2D] shadow-[0_18px_40px_rgba(45,45,45,0.04)] sm:px-7 sm:py-6">
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-3">
            <button
              type="button"
              onClick={() => navigate("/tracking/trackingpage/partners/catalogs")}
          ...
          {/* Search Bar and View Toggle - Only show view toggle in non-preview mode */}
          <div className="flex items-center gap-3">
            ...
            {/* View Toggle Buttons - Hidden in preview mode */}
            {!isPreviewMode && (
        ...
        {!isPreviewMode && !loading && !error && filteredProducts.length > 0 && (
          <div className="mt-6 flex-1 overflow-auto">
            {viewMode === "card" ? (
              /* Card View - Shows PDF preview with catalog name and download */
              <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
              ...
              /* List View - Shows only catalog name and download */
                ...
                {filteredProducts.map((product) => {
                  ...
                  return pdfFiles.map((file, idx) => (
                    ...
                    >
                      ...
                      {/* Content - Catalog Name Only */}
                      <div className="min-w-0 flex-1">
                        <h3 className="text-sm font-semibold text-[#2D2D2D] truncate">
                          {file?.name ? file.name.replace(/\.pdf$/i, "") : "Catalog"}
                        </h3>
                      </div>
