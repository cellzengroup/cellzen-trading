...
export default function DistributorCatalog() {
  ...
  useEffect(() => {
    const loadSharedProducts = async () => {
      try {
        ...
        // Filter products that are actually shared with distributors
        const sharedProducts = (result.data || []).filter(p => 
          p.share_to && p.share_to.distributors === true
        );
  ...
  const handleFolderClick = (folder) => {
    navigate(`/tracking/trackingpage/distributor/catalog-list`, { 
      state: { category: folder.category } 
    });
  };

  return (
    <DistributorPortal activePage="Catalogs">
      <div className="flex h-full flex-col overflow-hidden rounded-[2rem] bg-white px-5 py-5 text-[#2D2D2D] shadow-[0_18px_40px_rgba(45,45,45,0.04)] sm:px-7 sm:py-6">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h2 className="text-lg font-semibold text-[#412460]">Distributor Catalogs</h2>
