    ...
    <div ref={dropdownRef} className="relative">
      ...
      >
        <div className="flex h-10 w-10 items-center justify-center bg-[#412460] text-sm font-bold text-white rounded-full">
          {(adminUser?.name || "S").charAt(0)}
        </div>
        <div className="hidden sm:block text-left">
          <p className="text-xs font-semibold text-[#2D2D2D]">{adminUser?.name || "Supplier"}</p>
          <p className="text-[10px] text-[#2D2D2D]/45">{adminUser?.role || "Supplier"}</p>
        </div>
        <svg
...
const NAV_LINKS = [
  { title: "Dashboard", path: "/tracking/trackingpage/suppliers/dashboard", icon: "home" },
  { title: "Notices", path: "/tracking/trackingpage/suppliers/notices", icon: "notices" },
];
...
export default function SupplierPortal({ activePage, children }) {
  const navigate = useNavigate();
  const [sidebarCollapsed, setSidebarCollapsed] = useState(
    () => localStorage.getItem("suppliers_sidebar_collapsed") === "true"
  );
  ...
  useEffect(() => {
    localStorage.setItem("suppliers_sidebar_collapsed", String(sidebarCollapsed));
  }, [sidebarCollapsed]);
      ...
      >
        <aside className="flex h-full flex-col overflow-hidden bg-white p-4">
          <div className={`flex items-center ${sidebarCollapsed ? "justify-center" : "justify-between"} gap-3`}>
            <Link
              to="/tracking/trackingpage/suppliers/dashboard"
              aria-label="Suppliers dashboard"
          ...
          <header className="flex w-full flex-col gap-4 bg-white px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <h1 className="text-2xl font-semibold text-[#2D2D2D]">
                Hey there! {adminUser?.firstName || adminUser?.name?.split(" ")[0] || "Supplier"}, Have a wonderful Day
              </h1>
              <p className="mt-1 text-xs text-[#2D2D2D]/45">
                Manage quotation requests, product details, and shipment readiness.
              </p>
            </div>

            <div className="flex flex-wrap items-center gap-3">
              <NotificationBell noticesPath="/tracking/trackingpage/suppliers/notices" />
