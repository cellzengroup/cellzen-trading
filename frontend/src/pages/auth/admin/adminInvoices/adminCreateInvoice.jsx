...
export default function AdminCreateInvoice({ documentType = "PI" }) {
  ...
  const resolveRates = (tgt = currency) => {
    ...
    for (const r of entries) {
      // Rates are stored in the currency they were entered (rateCurrency), so
      // they never drift when the USD exchange rate changes. Legacy rows have no
      // rateCurrency and were stored in USD — fall back to 'USD' for those.
      const rateCurr = r.rateCurrency || "USD";
      const conv = (v) => convertCurrency(v, rateCurr, tgt);

      // New-format field: rateKg
      if (r.rateKg != null && kgRate === 0)
        kgRate = conv(r.rateKg);
      // New-format field: rateCBM
      if (r.rateCBM != null && cbmRate === 0)
        cbmRate = conv(r.rateCBM);
      // Border leg rate (new or old format)
      if (r.rateBorder != null && borderRate === 0)
        borderRate = conv(r.rateBorder);
              ...
              <div className="mb-4 flex items-center justify-between">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-sm font-bold text-[#412460]">
                      {parseFloat(formData.items.reduce((sum, item) => sum + (parseFloat(item.cbm) || 0), 0).toFixed(3))} CBM
                    </p>
                  </div>
                </div>
              </div>
