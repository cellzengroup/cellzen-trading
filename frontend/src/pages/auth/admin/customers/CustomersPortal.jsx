...
const NAV_LINKS = [
  { title: "Dashboard", path: "/admin/customers/dashboard", icon: "home" },
  { title: "Goods Tracking", path: "/admin/customers/goods-tracking", icon: "tracking" },
  { title: "Products", path: "/admin/customers/products", icon: "products" },
  { title: "Invoices", path: "/admin/customers/invoices", icon: "invoices" },
];
...
export default function CustomersPortal({ activePage, children }) {
  const navigate = useNavigate();
  const [sidebarCollapsed, setSidebarCollapsed] = useState(() => localStorage.getItem("customers_sidebar_collapsed") === "true");
  ...
  useEffect(() => {
    localStorage.setItem("customers_sidebar_collapsed", String(sidebarCollapsed));
  }, [sidebarCollapsed]);
      ...
      >
        <aside className="flex h-full flex-col overflow-hidden bg-white p-4">
          <div className={`flex items-center ${sidebarCollapsed ? "justify-center" : "justify-between"} gap-3`}>
            <Link to="/admin/customers/dashboard" aria-label="Customers Dashboard" className="flex min-w-0 items-center gap-2">
              <img src="/Images/DarkLogo.svg" alt="Cellzen Trading" className="h-9 w-auto shrink-0" />
              {!sidebarCollapsed && <span className="text-sm font-semibold text-[#412460]">Customers</span>}
            </Link>
            {!sidebarCollapsed && (
              <button
                type="button"
        ...
        <main className="flex h-full min-h-0 min-w-0 flex-col overflow-hidden">
          <header className="flex w-full flex-col gap-4 bg-white px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <h1 className="text-2xl font-semibold text-[#2D2D2D]">Customers Portal</h1>
              <p className="mt-1 text-xs text-[#2D2D2D]/45">Manage customer products, tracking, and invoices</p>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              <div>
                <p className="text-xs font-semibold">{adminUser?.name || "Customer Admin"}</p>
                <p className="text-[10px] text-[#2D2D2D]/45">{adminUser?.role || "Customer"}</p>
              </div>
              <button
                type="button"
